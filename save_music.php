...
if(!empty($_FILES['audio']['tmp_name'])){
    ...
    if(strpos($type,"audio/") > -1 || strpos($type2,"audio/") > -1){
        ...
        if(!$move){
            $resp['status'] = 'failed';
            $resp['msg'] = 'Không thể tải file audio.';
        ...
        $resp['status'] = 'failed';
        $resp['msg'] = 'Không đúng định dạng file audio.';
        $resp['error'] = $type;
        $resp['file'] = $_FILES;
    }
}
...
if(!isset($resp['status'])){
    ...
    if($save){
        ...
        if(!empty($_FILES['img']['tmp_name'])){
            ...
            if(strpos($type,"image/") > -1){
                ...
                if($move){
                    ...
                    $data['msg'].= "Không thể tải ảnh lên.";
                }
            }
        }
        ...
        $resp['status'] = 'failed';
        $resp['msg'] = 'Có lỗi khi thêm nhạc. Hãy kiểm tra lại!';
        $resp['error'] = $conn->error;
